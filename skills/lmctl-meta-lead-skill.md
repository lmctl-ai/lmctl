...
## Delegate to a Lead asynchronously — then ask for more in scope
A Lead's turn can run for minutes. Launch the blocking call in the background,
then let `lmctl more` wake you when tracked work completes or a delivered queue
receipt arrives:
```sh
lmctl chat "<teamA>.lmctl" Lead "coordinate the X change with your Coder+Reviewer" &
lmctl more --json
```
`more` is intentionally scoped. Default self-scope comes from
`LMCTL_SELF_SESSIONID` when running inside a member session. There is no
identity flag and no system-wide `more` scope; do not try to wake on unrelated
teams' completions.

If you remember older lmctl forms, read the removed-command block in the basic
Lead skill. Meta-Lead work now uses the current surface: `chat` and `more`.
...
`more` to flush idle lanes and collect finished receipts. Don't broadcast
